...
class BST{
	public:
		...
		void checkLeaveHeight(Node *head){
			
			if(count == height && head->left == NULL && head->right == NULL){
				totalleavesatthatheight++;
			}
			...
			if(head->left != NULL){
				count++;
				checkLeaveHeight(head->left);
			}
			
			
			if(head->right != NULL){
				count++;
				checkLeaveHeight(head->right);
			}
...
int main(){
	...
	do{
		...
		cout<<"7. Check At A Specific Height How Many Leaves Are Present"<<endl;
		...
		if(option == 1){
			...
			cout<<"Enter The Height At Which You Want To Find Leaves : ";
			cin>>T.height;
			
			T.count = 0;
			T.totalleavesatthatheight = 0;
			
			T.checkLeaveHeight(T.root);
			
			cout<<"At This Height "<<T.totalleavesatthatheight<<" Leaves Are Present"<<endl<<endl;
